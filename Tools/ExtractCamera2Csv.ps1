...
## Devices
##
$SqlQuery5 = "select IDDevice, IDHardware, Name, Description, Enabled, Settings from dbo.Devices where DeviceType = 'Camera' order by IDHardware "
...
$DevGrpMemberArray | Where-Object { $_.IDRecorderName -ne $Null  } | select IDRecorderName, DGName, DGLevel, DevName, DevEnabled, HWName, HWEnabled, HWDescription, VendorProdID, LoginID, URI, MacAddress, SerialNumber, DetectedModel  | Export-Csv -NoTypeInformation -Path $csvfilename 
